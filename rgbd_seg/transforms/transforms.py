...
class PadIfNeeded(albu.PadIfNeeded):
    ...
    def apply(self, img, pad_top=0, pad_bottom=0, pad_left=0, pad_right=0, **params):
        ...
        for i in range(0, channels, 3):
            stop = channels if i + 3 > channels else i + 3
            pad_img = F.pad_with_params(img[:, :, i:stop], pad_top, pad_bottom, pad_left, pad_right,
                                        border_mode=self.border_mode, value=self.value[i:stop])
            pad_imgs.append(pad_img)

        return np.concatenate(pad_imgs, axis=-1)
